...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8042
EXPOSE 8042
...
| `EXPOSE 8042` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8042"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8042"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15442"| POSTGRES
```
...
docker network create proyecto_construccion_de_software1_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software1_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_construccion_de_software1_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8042
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8042` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8042` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8042:8042`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8042`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software1` produce
la red `proyecto_construccion_de_software1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software1-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software1_default has active endpoints (name:"proyecto_construccion_de_software1-api-facturas-1" …)
```
